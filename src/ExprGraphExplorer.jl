...
const _matrix_svg_cache = Dict{String,_EmbeddedSVG}()
function _matrix_svg(value::AbstractMatrix)
    key = _fmt(value)
    return get!(_matrix_svg_cache, key) do
        matrix = Latexify.latexify(value; env=:array, fmt=_fmt)
        formula = typeof(matrix)("\\[\\mathrm{value} = " * matrix.s * "\\]")
        mktempdir() do directory
            name = joinpath(directory, "matrix")
            Latexify.render(
                formula,
            ...
            svg = "$name.svg"
            run(`dvisvgm --no-fonts --bbox=min -v 0 $name.dvi -o $svg`)
            source = read(svg, String)
            matched = match(r"<svg[^>]*viewBox='([^']+)'[^>]*>(.*)</svg>"s, source)
            isnothing(matched) && error("Could not read the SVG generated by dvisvgm")
            bounds = parse.(Float64, split(matched.captures[1]))
            prefix = "matrix$(length(_matrix_svg_cache) + 1)-"
            content = replace(matched.captures[2], "id='" => "id='$prefix")
            content = replace(content, "xlink:href='#" => "xlink:href='#$prefix")
            return _EmbeddedSVG(content, bounds...)
        end
    end
end

function _place_svg(svg::_EmbeddedSVG, x, y, width, height)
    scale = min(width / svg.width, height / svg.height)
    tx = x + (width - scale * svg.width) / 2 - scale * svg.min_x
    ty = y + (height - scale * svg.height) / 2 - scale * svg.min_y
    return "<g transform=\"translate($tx $ty) scale($scale)\">$(svg.content)</g>"
end
...
function render_svg(graph::ExprGraph, frame::Frame; width=1100, height=620, exam=false, responsive=true)
    ...
    println(io, "<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" $dimensions>")
    ...
    for node in order
        ...
        if frame.values[node] && node.value isa AbstractMatrix && length(node.value) <= 16
            println(io, _place_svg(_matrix_svg(node.value), x - 61, y - 3, 122, 36))
            value_y = y + 13
        else
        ...
        for (index, row) in enumerate(frame.metadata[node])
            ...
            println(io, "<text x=\"$x\" y=\"$metadata_y\" text-anchor=\"middle\" font-family=\"sans-serif\" font-size=\"16\" fill=\"#175cd3\">$(_escape(row.first)) = $(_escape(row.second))</text>")
        end
    end
    println(io, "</svg>")
